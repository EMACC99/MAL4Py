...
class _MalToken:
    ...
    async def _from_authorization_code(
...
class _MalAccount:
    ...
    def __init__(
        self, client_id: str, client_secret: str, mal_token: _MalToken | None = None
    ):
    ...
    async def refresh_token(self):
        ...
        if not (self.__mal_token is None):
            self.__mal_token = await _MalToken._from_refresh_token(
                self.__client_id, self.__client_secret, self.__mal_token
            )
        return self
...
class Auth:
    ...
    def __init__(
        self,
        client_id: str = "6114d00ca681b7701d1e15fe11a4987e",
        client_secret: str = "",
    ):
    ...
    def get_oauth_url(self, code_challenge: str) -> str:
        ...
        return (
            "%soauth2/authorize?response_type=code&client_id=%s&code_challenge=%s&state=RequestID42"
    ...
    async def authorize_with_refreshtoken(self, refresh_token: str) -> _MalAccount:
        ...
        MALTOKEN = await _MalToken._from_refresh_token(
            self.__client_id, self.__client_secret, refresh_token
        )
    ...
    async def authorize_with_code(self, code: str, code_challenge: str) -> _MalAccount:
        ...
        MALTOKEN = await _MalToken._from_authorization_code(
            self.__client_id, self.__client_secret, code, code_challenge
        )
        return _MalAccount(self.__client_id, self.__client_secret, MALTOKEN)
